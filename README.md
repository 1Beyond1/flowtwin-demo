...
- 后端按预计到站时刻生成站点占用率与 P50/P90 排队仿真预测；路线 ETA 和可行性使用统一的补能停靠账本：`排队 + 补能服务时长 + 支付驶离缓冲`。有端口状态输入时使用端口级离散事件仿真，当前 Demo 从已有演示占用率、容量和排队字段推导一份明确标注的端口快照，缺少端口状态时回退到聚合流量仿真。预测结果是 FlowTwin 演示仿真，不是能链企业实时数据。雨雪天气可按高德实况调整仿真排队（`weatherFactor`）。
...
- 评委模式新增轻量视觉 MVP：可分析内置 AI 生成的模拟摄像头画面，展示车辆、车位、排队、到站识别和模拟收据的结构化结果；可选本地 CV 服务适配 OpenCV/Paddle 运行时，未配置模型时明确降级，不把模拟结果写成真实摄像头识别。
...
- 演示仿真：站点占用率、到达率、服务率、价格、端口状态快照、按预计到站时刻计算的排队预测、优惠、订单和 ROI。端口快照中的总枪位、空闲枪位、充电中枪位、故障枪位、排队车辆和预计释放时间均为演示输入，不是能链企业实时经营数据。
