...
Node 主服务不依赖 Python、OpenCV 或 Paddle。直接进入“视觉”页即可运行内置合成演示；需要试验本地 CPU 适配器时，可按 [`cv-service/README.md`](cv-service/README.md) 启动 `127.0.0.1:5099`，再在私密配置中设置 `CV_SERVICE_URL`。模型权重不随项目提供，也不得提交到 GitHub。上传图片只在请求内存中处理，未启用推理时会显示“未执行视觉推理”，不会伪造识别结论。
