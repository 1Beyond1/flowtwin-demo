...
- 车主端比较最快、最可靠和最低成本三种目标，并解释驾驶、排队、补能服务、支付驶离、绕行和费用权衡；同一条路线结果可以同时获得多个目标标签，不人为伪造三条差异路线。长途非油服务提示会标明「主路线纯驾驶时长」与「途中休息建议」，避免误解为全程仅 2 小时。
...
Node 主服务不依赖 Python、OpenCV 或 Paddle。进入“视觉”页后，内置样例和上传的车牌图片/短视频都只有在本地 CV 服务可用时才会进入 PaddleOCR；可按 [`cv-service/README.md`](cv-service/README.md) 启动 `127.0.0.1:5099`，再在私密配置中设置 `CV_SERVICE_URL`。模型权重不随项目提供，也不得提交到 GitHub。上传图片只在请求内存中处理，并校验 PNG/JPEG/WebP 文件头；未启用推理或识别失败时会显示“未执行/未识别”，不会伪造车牌、车位或支付结论。
...
- 企业数据：当前已接入一份脱敏后的城市 × 油/电类型 × 小时需求先验，用于调整仿真到达强度和繁忙风险；该模型留出 WAPE 约 17.85%，不是具体高德 POI 的实时经营数据。没有可靠站点映射的候选会自动回退到演示输入。
