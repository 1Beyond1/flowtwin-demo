...
- 后端按预计到站时刻生成站点占用率与 P50/P90 等待仿真预测，并将对应预测等待纳入路线可行性和目标排序；预测结果是 FlowTwin 演示仿真，不是能链企业实时数据。雨雪天气可按高德实况调整仿真等待（`weatherFactor`）。
...
- 演示仿真：站点实时占用率、到达率、服务率、价格、按预计到站时刻计算的等待预测、优惠、订单和 ROI。上述内容不是能链企业实时经营数据。
