...
function stationFields(station, runId, snapshotAt, source, dataAsOf) {
  ...
  return {
    ...
    [names.snapshotAt]: text(snapshotAt, 80),
    ...
    [names.dataAsOf]: text(dataAsOf || snapshotAt, 80)
  };
}
...
function strategyFields(strategy, runId, strategyId, sourceStation, receivingStation, createdAt) {
  ...
  return {
    ...
    [names.approval]: "待确认",
    [names.createdAt]: text(createdAt, 80)
  };
}
...
function syncFields(job, status, error = "") {
  ...
  return {
    ...
    [names.startedAt]: text(job.startedAt, 80),
    [names.finishedAt]: status === "completed" || status === "error" ? new Date().toISOString() : "",
