...
function choose(strategy, stations, loads, allocations, deadline) {
  ...
  if (strategy === "flowtwin") {
    const scored = stations.map((station, index) => {
      ...
      const predictedTail = queueAt(station, index) + (station.tailEstimate ?? (1 - station.reliability) * 52);
...
function runStrategy(scenarios, strategy) {
  ...
  for (const scenario of scenarios) {
    ...
    if (strategy === "flowtwin") {
      ...
      // 现在只算成本：先求司机自己会去哪（realtime 口径，只看队列和距离），
      ...
      if (spontaneous !== index) {
        ...
        if (totalMinutes > alternative.distance * 2.7 + realisedQueue(alternative, spontaneous)) {
          compensatedTrips += 1;
...
export function validateStrategies({ seed = 20260719, trips = 1000, stations = null } = {}) {
  ...
  return {
    ...
    strategies: results,
    metricDefinitions: {
      averageWait: "所有行程到站前实际等待时间的平均值（分钟）",
      ...
      divertedTrips: "FlowTwin 的推荐与用户自发选择（仅看队列和距离）不一致的行程数",
    ...
    methodology: {
      scenarioGeneration: {
        ...
        waitMultiplier: "每次行程按基础等待乘以 0.75–1.25",
      ...
      formulas: {
        queue: "基础等待 + 当前分配负载 / 站点容量 × 14 + 尾部风险真值",
        totalMinutes: "站点距离 × 2.7 + 实际等待",
        flowTwinScore: "0.72 ×（队列 + 尾部风险预测值）+ 0.18 × 距离 + 2.2 × 价格 + 到达时限风险；近似同分时优先分配压力更低的站点",
        roi: "券成本 = 4.5 元 × 需补偿单数；毛利 = (FlowTwin 准点率 − realtime 准点率) × 样本量 × 站点平均毛利"
      },
      strategies: {
        nearest: "仅按距离最短选站",
        cheapest: "仅按价格最低选站",
        realtime: "按当前队列等待 + 距离加权选站；也用作 FlowTwin 的反事实基线（用户不被干预时的自发选择）",
