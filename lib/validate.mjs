...
export function validateStrategies({ seed = 20260719, trips = 1000, stations = null } = {}) {
  ...
  return {
    ...
    methodology: {
      ...
      formulas: {
        queue: "基础等待 + 当前分配负载 / 站点容量 × 14 + 尾部风险项",
        totalMinutes: "站点距离 × 2.7 + 预计等待",
        flowTwinScore: "0.72 × 预测尾部等待 + 0.18 × 距离 + 2.2 × 价格 + 到达时限风险；近似同分时优先分配压力更低的站点"
      },
      strategies: {
        nearest: "仅按距离最短选站",
        cheapest: "仅按价格最低选站",
        realtime: "按当前队列等待 + 距离加权选站",
        flowtwin: "综合队列尾部风险、时限、距离、价格和站点分配压力选站"
      }
    },
    assumptions: "固定种子合成出行样本，仅用于可复现的策略对比，不代表能链真实经营数据。"
  };
}
