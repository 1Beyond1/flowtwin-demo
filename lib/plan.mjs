...
// Voice transcription can place a full stop between the destination cue and
// the actual place name: “我想去。南京大学。” Treat it as a speech
// segmentation artefact, not as the end of the destination phrase.
function normalizeDestinationBoundaries(value) {
  return String(value ?? "").replace(
    /((?:前往|去|抵达|目的地(?:是)?|到(?!达)|导航(?:到|去)))\s*[。！？!?]+\s*/g,
    "$1 "
  );
}
...
function extractServiceRequests(text) {
  const requests = [];
  const seen = new Set();
  const add = (request) => {
    const key = `${request.service}|${request.name || ""}|${request.location || ""}`;
    if ((request.name || request.location) && !seen.has(key)) {
      seen.add(key);
      requests.push(request);
    }
  };
  for (const match of text.matchAll(new RegExp(KNOWN_SERVICE_NAME_PATTERN.source, "g"))) {
    add({ service: "餐饮", name: normalizeServiceName(match[0]) || match[0] });
  }
  // Preserve a second concrete request such as “再找个咖啡店” as a bounded
  // service location. It is intentionally not treated as a destination or a
  // named brand, but dropping it would make a composite request look like a
  // single-stop request.
  if (/咖啡(?:店|馆)/.test(text)) add({ service: "餐饮", location: "咖啡店" });
  return requests.slice(0, 4);
}

function removalCue(text) {
  if (/取消|删(?:掉|除)?|去掉|移除/.test(text)) return true;
  // “尽量不要下高速/不要走国道” is a route preference, not a request to
  // remove a previously selected service stop. Only treat a negation as a
  // removal when it is followed by an explicit stop/service target.
  return /(?:不要|别|不再|不需要|不用)\s*(?:再|继续|去|加|安排|选择|保留)?\s*(?:这(?:个|家)?|那(?:个|家)?|该)?\s*(?:麦当劳|肯德基|星巴克|瑞幸|汉堡王|必胜客|海底捞|老乡鸡|德克士|喜茶|奈雪|餐饮|餐厅|咖啡(?:店|馆)?|休息|休息区|洗车|补能|充电|加油|加气|加氢)/.test(text);
}
...
function validatedActions(modelActions, localActions, { aiDestination = null } = {}) {
  ...
  for (const action of model) {
    ...
    // The one narrow exception is a destination recovery: when the local rule
    // found no destination at all, an AI NEW_TRIP/CHANGE_DESTINATION action
    // matching the AI candidate may pass this gate. AMap geocoding below still
    // has to confirm that candidate before a route can be generated.
    const isDestinationRecovery = Boolean(aiDestination)
      && (action.type === "NEW_TRIP" || action.type === "CHANGE_DESTINATION")
      && action.destination === aiDestination
      && !local.some((candidate) => candidate.type === action.type);
    if (!isDestinationRecovery && !localActionMatchesText(action, local)) continue;
...
function enrichConcreteServiceActionNames(actions, text) {
  ...
  const enriched = normalized.map((action) => action.type === "ADD_SERVICE" && action.service === service && !action.name && !action.location
...
function buildLocalActions(text, context, draft) {
  ...
  if (canApplyToTrip && removal) {
    ...
    if (services.length && hasAdditionCue) {
      const requests = extractServiceRequests(text);
      if (requests.length) {
        actions.push(...requests.map((request) => ({ type: "ADD_SERVICE", ...request })));
        const requestedTypes = new Set(requests.map((request) => request.service));
        for (const service of services) {
          if (!requestedTypes.has(service)) actions.push({ type: "ADD_SERVICE", service });
        }
      } else {
        for (const service of services) {
          actions.push({
            type: "ADD_SERVICE",
            service,
            ...(service === services[0] && extractServiceName(text) ? { name: extractServiceName(text) } : {}),
            ...(service === services[0] && extractServiceLocation(text) ? { location: extractServiceLocation(text) } : {})
          });
        }
      }
    }
...
function buildAnalysis(text, context, localResult, {
  ...
  } else if (destinationResolution === "ai-fallback") {
    factors.push(analysisFactor("destination-evidence", "目的地证据", "pass", 22, "本地规则未直接提取到目的地，语义模型复核后交由高德确认"));
  ...
  if (localResult.clarificationNeeded && destinationResolution !== "ai-fallback") {
    factors.push(analysisFactor("route-intent", "路线意图", "fail", -30, "本地规则无法形成可规划的目的地"));
  } else if (destinationResolution === "ai-fallback") {
    factors.push(analysisFactor("route-intent", "路线意图", "pass", 12, "语义模型已补充目的地，仍需通过地理编码确认后才能规划"));
  } else {
...
function contextForPrompt(context) {
  ...
  // explicitDestination / destinationLocation / destinationCity are used by
  // the destination picker path and must not be stripped before resolvePlace.
  const allowed = ["origin", "destination", "currentOrigin", "currentDestination", "currentServices", "hasPlannedRoute", "arrivalDeadline", "minArrivalSoc", "energyType", "priority", "maxDetourKm", "services", "explicitDestination", "destinationLocation", "destinationCity", "originLocationSource", "originCity"];
...
export async function parseTripIntent({ message, context = {}, config, fetchImpl = fetch } = {}) {
  const safeMessage = normalizeDestinationBoundaries(cleanText(message, MAX_MESSAGE_LENGTH));
  const promptContext = contextForPrompt(context);
  ...
  // Keep the browser-provided coordinate for deterministic route resolution,
  // but do not pass the precise location into an external LLM prompt.
  const explicitOriginCoordinate = parseCoordinate(context.originLocation);
  if (explicitOriginCoordinate) safeContext.originLocation = explicitOriginCoordinate;
  ...
  // The local parser remains the first gate. Only when it found no usable
  // destination may an AI result supply one; this keeps explicit local
  // evidence authoritative while still allowing noisy speech such as
  // “我准备去那个学校” to be recovered semantically. The candidate is not
  // trusted as a route yet: resolvePlace below must still confirm it with AMap.
  const aiDestination = !localResult.destination
    && aiResult?.destination
    && aiResult.requestMode === "new_trip"
    ? aiResult.destination
    : null;
  const destinationResolution = localResult.destination
    ? "rule"
    : aiDestination
      ? "ai-fallback"
      : "unresolved";
  const resolvedDestination = localResult.destination || aiDestination || null;
  const safeActions = enrichConcreteServiceActionNames(
    aiResult
      ? validatedActions(aiResult.actions, localResult.actions, { aiDestination })
      : localResult.actions,
    safeMessage
  );
  const plan = aiResult ? normalizePlan({
    ...
    clarificationNeeded: !resolvedDestination,
    aiUsed: true,
  ...
  if (explicitOriginCoordinate) {
    // A selected browser location is an explicit coordinate, not a place name
    // for the model to reinterpret or geocode again.
    plan.origin = cleanText(safeContext.origin, 80) || "用户当前位置";
  }

  const originGeo = explicitOriginCoordinate
    ? {
        coordinate: explicitOriginCoordinate,
        source: cleanText(safeContext.originLocationSource, 80) || "用户当前位置（浏览器定位）",
        name: cleanText(safeContext.origin, 80) || "用户当前位置",
        city: cleanText(safeContext.originCity, 40) || null,
        score: 100
      }
    : plan.origin === DEFAULT_ORIGIN.name
  ...
  if (plan.destination && explicitCoordinate) {
    destinationGeo = {
      ...
      city: cleanText(safeContext.destinationCity, 40) || null,
      score: 100
    };
...
export function formatPlanResponse(result) {
  const {
    ...
    aiFailureCode = null,
    analysis = null,
    destinationResolution: suppliedDestinationResolution = null,
    ...parsed
  } = result || {};
  ...
  const destinationResolution = suppliedDestinationResolution
    || (resolvedDestinationName || parsed.destination ? "rule" : "unresolved");
  const publicLocationMeta = (location, fallbackName) => ({
    name: cleanText(location?.name || fallbackName || "", 80) || null,
    city: cleanText(location?.city || "", 40) || null,
    district: cleanText(location?.district || "", 40) || null
  });
  return {
    ...
    // City metadata lets the operator view describe the actual trip corridor
    // instead of relabelling every intercity plan as the origin's local area.
    // Only short public place labels are exposed; no provider payload is passed.
    locationMeta: {
      origin: publicLocationMeta(locations.origin, parsed.origin),
      destination: needsPick
        ? publicLocationMeta(null, userQuery)
        : publicLocationMeta(locations.destination, resolvedDestinationName || parsed.destination)
    },
