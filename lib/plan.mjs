...
function localParse(message, context = {}) {
  ...
  const soc = cleanNumber(text.match(/(\d{1,3})\s*%/)?.[1], 5, 100);
...
function contextForPrompt(context) {
  ...
  const allowed = ["origin", "destination", "departureTime", "arrivalDeadline", "soc", "energyType", "priority", "maxDetourKm", "services"];
    ...
    const response = await fetchImpl(`${config.aiBaseUrl}/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "你是 FlowTwin 出行规划解析器。只输出合法 JSON，不要 Markdown，不要解释。字段必须为 origin,destination,departureTime,arrivalDeadline,soc,energyType,priority,maxDetourKm,services,clarificationNeeded,assistantReply。energyType 只能是 electric、fuel、mixed、unknown；priority 只能是 on_time、fastest、cheapest、wait、balanced。缺失字段用 null，缺少目的地时 clarificationNeeded=true。"
          },
          {
            role: "user",
...
export async function parseTripIntent({ message, context = {}, config, fetchImpl = fetch } = {}) {
  ...
  const plan = aiResult ? normalizePlan({
    ...
    soc: aiResult.soc ?? localResult.soc,
