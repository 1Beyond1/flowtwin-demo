...
function buildAnalysis(text, context, localResult, { aiRequested = false, aiUsed = false } = {}) {
  const planned = hasPlannedRoute(context);
  const explicitDestination = Boolean(findDestination(text));
  const services = serviceEvidence(text);
  ...
  return {
    mode: aiRequested ? (aiUsed ? "ai" : "local-fallback") : "local",
    score,
    level: score >= 75 ? "high" : score >= 50 ? "medium" : "low",
    factors,
    ai: {
      requested: Boolean(aiRequested),
      used: Boolean(aiUsed),
...
export async function parseTripIntent({ message, context = {}, config, fetchImpl = fetch } = {}) {
  ...
  return {
    ...plan,
    analysis: buildAnalysis(safeMessage, safeContext, localResult, {
      aiRequested,
...
function sanitizeAnalysis(value) {
  ...
  const level = ["high", "medium", "low"].includes(value.level) ? value.level : "low";
  const mode = ["local", "ai", "local-fallback"].includes(value.mode) ? value.mode : "local";
  return {
    mode,
    score,
    level,
    factors,
    ai: {
      requested: Boolean(value.ai?.requested),
