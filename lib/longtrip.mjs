...
function evaluateSequence(sequence, input) {
  ...
  for (let index = 0; index < sequence.length; index += 1) {
    ...
    const stationSnapshot = station.inputSnapshot || station.forecastInputSnapshot || {};
    const stationStateNumber = (key) => {
      const direct = Number(station?.[key]);
      if (Number.isFinite(direct)) return Number(direct.toFixed(1));
      const snapshotValue = Number(stationSnapshot?.[key]);
      return Number.isFinite(snapshotValue) ? Number(snapshotValue.toFixed(1)) : null;
    };
    const stationState = stationStateNumber("totalPorts") !== null
      ? {
        totalPorts: stationStateNumber("totalPorts"),
        idlePorts: stationStateNumber("idlePorts"),
        availablePorts: stationStateNumber("availablePorts"),
        reservedPorts: stationStateNumber("reservedPorts"),
        chargingPorts: stationStateNumber("chargingPorts"),
        faultPorts: stationStateNumber("faultPorts"),
        waitingVehicles: stationStateNumber("waitingVehicles"),
        queueVehicles: stationStateNumber("queueVehicles"),
        reservationQueueAhead: stationStateNumber("reservationQueueAhead"),
        averageSessionMinutes: stationStateNumber("averageSessionMinutes"),
        availabilitySource: station.availabilitySource || stationSnapshot.availabilitySource || null,
        queueSource: station.queueSource || stationSnapshot.queueSource || null
      }
      : {};
    stops.push({
      id: station.id,
