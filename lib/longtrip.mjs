...
const MAX_STOPS = 6;
// Six remains only as the legacy default for direct API callers that do not
// explicitly opt into route-length-aware planning. The browser planner uses
// adaptiveMaxStops and does not impose a product-level stop-count limit.
...
// evaluation budget below, so this is not a stop-count limit.
...
function enumerateSequences(candidates, maxStops, minStops = 0, limit = Infinity) {
  const sequences = [[]];
...
function buildAdaptiveGreedySequence(candidates, totalDistanceKm, input = {}) {
  const energyType = ENERGY_TYPE_KEYS.has(input.energyType) ? input.energyType : "electric";
  const profile = getEnergyProfile(energyType);
  const soc = clamp(finite(input.soc, 0), 0, 100);
...
export function buildLongTripPlans(input = {}) {
  ...
  const candidateBudget = input.adaptiveMaxStops === true ? rankedCandidates.length : MAX_CANDIDATES;
  const candidates = selectCoverageCandidates(rankedCandidates, totalDistanceKm, candidateBudget, input);
  // In adaptive mode the number of selected candidates is the natural search
  // boundary. There is no fixed six/twelve-stop product limit; the sequence
  // can contain as many stops as the corridor candidate set requires. The
  // legacy branch remains bounded for older direct API callers.
  ...
  if (directRoute) {
    ...
    const seed = buildAdaptiveGreedySequence(candidates, totalDistanceKm, input);
    const alternatives = enumerateSequences(candidates, maxStops, Math.max(1, minimumStops), MAX_ADAPTIVE_SEQUENCE_EVALUATIONS);
    const seen = new Set();
    sequences = [];
    if (seed) {
      const signature = seed.map((station) => station.id).join("|");
      seen.add(signature);
      sequences.push(seed);
    }
    alternatives.forEach((sequence) => {
      const signature = sequence.map((station) => station.id).join("|");
