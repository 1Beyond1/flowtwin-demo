...
const MAX_STOPS = 6;
const ADAPTIVE_MAX_STOPS = 12;
// Six stops remains the normal planning budget.  The browser can opt into the
// adaptive budget for an extreme intercity trip (for example Beijing → Lhasa)
// after the energy model proves that six stops cannot cover the distance.  The
// upper bound is deliberately explicit: we never let an arbitrary client turn
// the combinatorial search into an unbounded request.
// Keep 18 route-coverage candidates (two per ninth of the corridor) so the
// optimiser still sees every part of a national route.  The adaptive search is
// still bounded to C(18, 0..12) sequences and is cheap for the compact scalar
// station payload used by this API.
...
export function buildLongTripPlans(input = {}) {
  const totalDistanceKm = Math.max(0, finite(input.distanceKm));
  const stopCap = input.adaptiveMaxStops === true ? ADAPTIVE_MAX_STOPS : MAX_STOPS;
  const maxStops = clamp(Math.floor(finite(input.maxStops, MAX_STOPS)), 0, stopCap);
