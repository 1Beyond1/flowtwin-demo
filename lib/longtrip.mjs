...
function evaluateSequence(sequence, input) {
  ...
  return {
    ...
    confidenceScore: stops.length
      ? Number((stops.reduce((sum, stop) => sum + (Number(stop.confidenceScore) || 0), 0) / stops.length).toFixed(0))
      : null,
    confidenceLevel: stops.length
      ? (stops.some((stop) => stop.confidenceLevel === "low") ? "low" : stops.some((stop) => stop.confidenceLevel === "medium") ? "medium" : "high")
      : null,
    confidenceLabel: stops.length
      ? (stops.some((stop) => stop.confidenceLevel === "low") ? "低（演示口径）" : stops.some((stop) => stop.confidenceLevel === "medium") ? "中（演示口径）" : "高（演示口径）")
      : null,
    horizonMinutes
  };
}
