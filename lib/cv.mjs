...
export function visionHealthSummary(config = {}) {
  const serviceConfigured = Boolean(config.cvServiceUrl);
  return {
    // `configured` only means that an endpoint was supplied. It must not be
    // interpreted as OCR readiness; readiness requires a health probe and a
    // real inference result.
    configured: serviceConfigured,
    serviceConfigured,
    service: serviceConfigured ? "optional-local-service" : "not-configured",
    serviceReachable: null,
    runtimeAvailable: null,
    modelLoaded: null,
    inferenceReady: null,
    status: serviceConfigured ? "configured-unchecked" : "not-configured",
