...
function forecastPortStation(station, context) {
  ...
  const forecast = context.points.map((minute) => {
    ...
    const queuedOnUsablePorts = Math.min(
      usablePorts,
      snapshot.queueVehicles + snapshot.reservationQueueAhead + preArrivalVehicles
    );
    const occupancy = clamp(
      (snapshot.chargingPorts + snapshot.reservedPorts + queuedOnUsablePorts) / Math.max(1, snapshot.totalPorts),
      0.05,
      0.99
    );
  ...
  return {
    ...
    horizonMinutes: context.horizonMinutes,
    simulation: true,
    explanation: `${station.enterprisePrior ? "企业需求先验 + " : ""}simulation 仿真：使用总枪位、空闲/预约/充电中/故障状态、已到站等待车辆和到站前预约队列，先安排队列再安排预计到达车辆，估算到站等待；dataAsOf=${dataAsOf}，预约队列与补能位状态当前为${snapshot.dataSource || "演示输入"}，企业先验仅为城市×油电类型×小时需求，不代表具体站点实时经营数据。`
  };
}
