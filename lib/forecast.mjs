...
function hasPortInput(station) {
  const totalValue = Number(station?.totalPorts);
  const hasTotal = hasInputValue(station, "totalPorts") && Number.isFinite(totalValue) && totalValue > 0;
  const hasStateField = PORT_STATE_FIELDS.some((key) => (
    hasInputValue(station, key) && Number.isFinite(Number(station[key]))
  ));
  ...
  const stateTotal = PORT_STATE_FIELDS.reduce((sum, key) => {
    if (!hasInputValue(station, key) || !Number.isFinite(Number(station[key]))) return sum;
    return sum + boundedInteger(station[key], 0, 0, MAX_PORTS);
  }, 0);
...
function normalizePortSnapshot(station, legacyServiceRate) {
  ...
  const totalValue = Number(station?.totalPorts);
  const explicitTotal = hasInputValue(station, "totalPorts") && Number.isFinite(totalValue) && totalValue > 0;
  const totalPorts = explicitTotal
    ? boundedInteger(station.totalPorts, 1, 1, MAX_PORTS)
...
function forecastPortStation(station, context) {
  ...
  return {
    ...
    horizonMinutes: context.horizonMinutes,
    simulation: true,
    explanation: `${station.enterprisePrior ? "企业需求先验 + " : ""}simulation 仿真：使用 totalPorts、idlePorts、chargingPorts、faultPorts、queueVehicles、estimatedReleaseMinutes、averageSessionMinutes，先安排当前排队车辆，再安排到站偏移前的预计到达车辆，估算到站等待；dataAsOf=${dataAsOf}，企业先验仅为城市×油电类型×小时需求，不代表具体站点实时经营数据。`
  };
}
