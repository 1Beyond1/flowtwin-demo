...
export function simulateOperator(input = {}) {
  ...
  const responseRate = discount / (discount + 7);
  ...
  const desiredDiversion = pressureDemand * switchRate;
  const diverted = Math.min(desiredDiversion, targetSpare * 0.78);
  ...
  const latentDemandRate = clamp(0.015 + responseRate * 0.055, 0, 0.075);
  const incremental = Math.min(totalDemand * latentDemandRate, remainingSpare * 0.65);
  ...
  const discountCost = acceptedOrders * discount;
  const incrementalGrossProfit = incremental * target.margin + diverted * target.margin * 0.35;
  const roi = discountCost > 0 ? incrementalGrossProfit / discountCost : 0;
  return {
