...
export function simulateOperator(input = {}) {
  ...
  // 发券/承接的权限只约束目标站；用户原本可能前往任何被平台观测到的补能站。
  // 不能把“来源站不可控”误判成“这部分需求不可被引导”，否则站外高峰压力
  // 永远无法进入分流计算，演示结果会被错误地压成 0。
  const sourceStations = target ? baseStations.filter((station) => station.id !== target.id) : [];
  ...
  const after = baseStations.map((station) => {
    ...
    const demand = target && station.id === target.id
      ...
      // 来源站可以是导航观察站；策略只改变用户选择，不修改该站的经营配置。
      : target
  ...
  return {
    ...
    execution: {
      ...
      boundary: "仅向 partner、controllable、couponEligible、merchantAccepted 全部为 true 的承接站发券；来源站可为任意可观察补能站，策略只模拟用户选择变化，不修改来源站经营配置。"
    },
